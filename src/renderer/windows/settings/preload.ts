...
contextBridge.exposeInMainWorld("ytmd", {
  ...
  store: {
    ...
    setMany: (values: { [TKey in keyof StoreSchema]?: StoreSchema[TKey] extends object ? Partial<StoreSchema[TKey]> : StoreSchema[TKey] }) =>
  ...
  safeStorage: {
    decryptString: async (value: string | null) => await ipcRenderer.invoke("safeStorage:decryptString", value),
  ...
  handleUpdateError: (callback: (event: Electron.IpcRendererEvent) => void) => ipcRenderer.on("app:updateError", callback),
  ...
  listPlugins: async () => await ipcRenderer.invoke("plugins:list")
});
